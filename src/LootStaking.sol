...
contract LootStaking is Ownable {
    ...
    event LootBagsStaked(
        address indexed _owner,
        uint256 indexed _epoch,
        uint256[] _bagIds
    );
    event MLootBagsStaked(
        address indexed _owner,
        uint256 indexed _epoch,
        uint256[] _bagIds
    );
    event LootRewardsClaimed(
        address indexed _owner,
        uint256 _amount,
        uint256 indexed _epoch,
        uint256[] _bagIds
    );
    event MLootRewardsClaimed(
        address indexed _owner,
        uint256 _amount,
        uint256 indexed _epoch,
        uint256[] _bagIds
    ...
    /*///////////////////////////////////////////////////////////////
                             STRUCTS
    //////////////////////////////////////////////////////////////*/
    // Struct used to circumvent stack too deep error.
    struct ClaimEpochsInfo {
        uint256[] epochs;
        uint256 epochsLength;
        // 2-bit map. 0x01 = keep current epoch, 0x10 = keep next epoch, 0x11 = keep both.
        uint8 keepCurrentOrNextEpoch;
    }
    ...
    constructor(
        ...
        lootWeightsByEpoch[0] = _lootWeight;
        mLootWeightsByEpoch[0] = _mLootWeight;
    }
    ...
    function setStakingStartTime(uint256 _startTime) external onlyOwner {
        ...
        if (_startTime < block.timestamp + epochDuration) {
    ...
    function setWeightsForEpoch(
        ...
        if (_epoch <= currentEpoch || _epoch > numEpochs) {
            revert EpochInvalid();
        }
        if (_lootWeight + _mLootWeight != 1e4) {
            revert WeightsInvalid();
        }
    ...
    function notifyRewardAmount(uint256 _amount) external onlyOwner {
        ...
        if (AGLD.balanceOf(address(this)) < _amount) {
            revert RewardsNotReceived();
        }
    ...
    function _signalStake(
        ...
        for (uint256 i = 0; i < length; ) {
            bagId = _ids[i];
            if (_nftToken.ownerOf(bagId) != msg.sender) {
                revert NotBagOwner();
            }
            if (stakedNFTsByEpoch[signalEpoch][bagId]) {
                revert BagAlreadyStaked();
            }
    ...
    /// @dev Given a set of IDs, we loop through each ID and then loop
    /// through each epoch that ID was staked in. For each epoch, we add the reward
    /// share for that epoch to the total rewards. Finally, we clear out their claim
    /// to a reward and then transfer the rewards.
    ...
    function _claimRewards(
        ...
        ClaimEpochsInfo memory claimEpochsInfo; // cache value
        uint256 defaultWeight = nftWeights[0]; // cache value
        ...
        for (uint256 i = 0; i < _ids.length; ) {
            ...
            claimEpochsInfo.epochs = epochsByNFTId[bagId];
            claimEpochsInfo.epochsLength = claimEpochsInfo.epochs.length;
            claimEpochsInfo.keepCurrentOrNextEpoch = 0x0;

            if (claimEpochsInfo.epochsLength > 0) {
                for (uint256 j = 0; j < claimEpochsInfo.epochsLength; ) {
                    if (claimEpochsInfo.epochs[j] < currentEpoch) {
                        // Proper ERC-20 implementation will cap total supply at uint256 max.
                        unchecked {
                            rewards += getRewardPerEpochPerBag(
                                rewardPerEpoch,
                                getWithDefault(
                                    nftWeights[claimEpochsInfo.epochs[j]],
                                    defaultWeight
                                ),
                                numNFTsStakedByEpoch[claimEpochsInfo.epochs[j]]
                            );
                        }
                    } else if (claimEpochsInfo.epochs[j] == currentEpoch) {
                        claimEpochsInfo.keepCurrentOrNextEpoch |= 0x01;
                    } else if (claimEpochsInfo.epochs[j] == currentEpoch + 1) {
                        claimEpochsInfo.keepCurrentOrNextEpoch |= 0x10;
                    }
                ...
                if (claimEpochsInfo.keepCurrentOrNextEpoch == 0x11) {
                    epochsByNFTId[bagId] = [currentEpoch, currentEpoch + 1];
                } else if (claimEpochsInfo.keepCurrentOrNextEpoch == 0x01) {
                    epochsByNFTId[bagId] = [currentEpoch];
                } else if (claimEpochsInfo.keepCurrentOrNextEpoch == 0x10) {
                    epochsByNFTId[bagId] = [currentEpoch + 1];
                } else {
    ...
    function getCurrentEpoch() public view returns (uint256 currentEpoch) {
        if (stakingStartTime == 0 || block.timestamp < stakingStartTime) {
            return 0;
        }

        currentEpoch =
            ((block.timestamp - stakingStartTime) / epochDuration) +
            1;
    }

    /// @notice Gets the bag reward weights for an epoch. If weights are unset, then
    /// the first weights are returned.
    ...
    function getWeightsForEpoch(uint256 _epoch)
        public
        view
        returns (uint256 lootWeight, uint256 mLootWeight)
    {
        lootWeight = getWithDefault(
            lootWeightsByEpoch[_epoch],
            lootWeightsByEpoch[0]
        );
        mLootWeight = getWithDefault(
            mLootWeightsByEpoch[_epoch],
            mLootWeightsByEpoch[0]
        );
    }
    ...
    function getClaimableRewardsForLootBag(uint256 _id)
        external
        view
        returns (uint256 rewards)
    {
        rewards = _getClaimableRewardsForEpochs(
            lootWeightsByEpoch,
            epochsByLootId[_id],
            numLootStakedByEpoch,
            lootWeightsByEpoch[0]
    ...
    /// @param _id The bag to calculate rewards for.
    /// @return rewards Claimable rewards for the bag.
    function getClaimableRewardsForMLootBag(uint256 _id)
        external
        view
        returns (uint256 rewards)
    {
    ...
    function getRewardsForEpoch(uint256 _epoch)
        public
        view
        returns (uint256 lootRewards, uint256 mLootRewards)
    {
        uint256 rewardPerEpoch = getTotalRewardPerEpoch();
        (uint256 lootWeight, uint256 mLootWeight) = getWeightsForEpoch(_epoch);
        lootRewards = Math.mulDiv(rewardPerEpoch, lootWeight, 1e4);
        mLootRewards = Math.mulDiv(rewardPerEpoch, mLootWeight, 1e4);
    }
    ...
    /// @param defaultWeight First epoch weight for the collection.
        ...
        for (uint256 i = 0; i < epochsLength; ) {
            ...
            if (epoch < currentEpoch) {
                ...
                unchecked {
                    rewards += getRewardPerEpochPerBag(
                        rewardPerEpoch,
                        getWithDefault(nftWeights[epoch], defaultWeight),
                        numNFTsStakedByEpoch[epoch]
                    );
                }
            }
    ...
    /*///////////////////////////////////////////////////////////////
                             PRIVATE HELPERS
    //////////////////////////////////////////////////////////////*/
    function getWithDefault(uint256 a, uint256 b)
        internal
        pure
        returns (uint256)
    {
